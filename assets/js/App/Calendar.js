        ...
        function (_Site) {
            ...
            babelHelpers.createClass(AppCalendar, [{
                ...
                value: function handleFullcalendar() {
                    ...
                    var myOptions = {
                        ...
                        select: function select() {
                            $('#addNewEvent').modal('show').one('hidden.bs.modal', function (e) {
                        ...
                        eventClick: function eventClick(event) {
                            ...
                            $('#editEventForm .event-success-message').hide();
                            $('#editEventForm .event-danger-message').hide();

                            $('#duration').val(event.appointment_duration);
                            $('#idSlot').val(event.idSlot);
                            $('#idAppoint').val(event.idAPPOINTMENT);
                        ...
                        eventDrop: function eventDragStop(event) {
                            console.log(event);
                            $('#duration').val(event.appointment_duration);
                            $('#idSlot').val(event.idSlot);
                            $('#idAppoint').val(event.idAPPOINTMENT);
                        ...
                        droppable: true,
                        drop:function (date, resource, allDay,jsEvent,view) {
                            var $this=$(this);
                            var data={
                                idAPPOINTMENT: parseInt($this.attr('data-idappointment')),
                            ...
                            $this.data('event',{
                                ...
                                appointment_duration:$this.attr('data-duration'),
                                idSlot:0,
                ...
                value: function handleEventList() {
                    ...
                    $('#SubmitAddEventForm').click(function (e) {
                        if(AddEventFormValidation()){
                            var formData=$('#addNewEventForm').serialize();
                            $.ajax({
                                method:"post",
                                // dataType:'json',
                                data:formData,
                                url:$('#addNewEventForm').attr('action'),
                                success:function (e) {
                                    console.log(e);
                                    appendEventToLeftPanel(3);
                                    $('#addNewEventForm').find('.event-success-message').slideDown(300)
                                    setTimeout(function (){
                                ...
                                error:function (err) {
                                    console.log(err);
                                    $('#addNewEventForm').find('.event-danger-message').slideDown('slow')
                                }
                            });
                        }
                    ...
                    $('.calendar-list .calendar-event').each(function () {
                        ...
                        //     title: $this.data('title'),
                        //     stick: $this.data('stick'),
                        //     idAPPOINTMENT:$this.data('idAPPOINTMENT'),
                        //     idSLOT:0,
                        //     start:"",
                        //     end:"",
                        ...
                        $this.draggable({
                            ...
                            helper: function helper() {
                                return `<a class="fc-day-grid-event fc-event fc-start fc-end" 
                                                style="background-color:${$this.data('color')};border-color:${(0, _Config.colors)(color[0], color[1])}">
                ...
                value: function handleListItem() {
                    ...
                    $(document).ready(function () {
                        $.ajax({
                            ...
                            success:function(result) {
                                var slots=JSON.parse(result);
                                $('#calendar').fullCalendar('events', slots);
